...
async function main() {
  ...
  try {
    ...
    if (drawResult.rows.length > 0) {
      console.log(`期數 ${periodNumber} 的開獎記錄已存在:`);
      drawData = drawResult.rows[0];
      console.log(drawData);
    } else {
      // 生成隨機開獎結果
      const drawNumbers = {
        numbers: Array.from({ length: 10 }, () => Math.floor(Math.random() * 10) + 1)
      };
      console.log('\n生成新的開獎結果:', drawNumbers);
      
      // 插入開獎記錄
      const now = new Date();
      const insertResult = await client.query(
        'INSERT INTO draw_records (period, result, draw_time, created_at) VALUES ($1, $2, $3, $4) RETURNING *',
        [periodNumber, JSON.stringify(drawNumbers), now, now]
      );
      drawData = insertResult.rows[0];
      console.log(`已插入開獎記錄，期數: ${periodNumber}`);
    }
    
    // 解析開獎結果
    const drawNumbers = JSON.parse(drawData.result).numbers || [];
    console.log('\n開獎號碼:', drawNumbers);
    ...
    if (betsResult.rows.length === 0) {
      console.log('沒有未結算的下注記錄，將檢查已結算的記錄');
      ...
      console.log(`期數 ${periodNumber} 的已結算下注記錄:`, settledResult.rows.length);
