// simulate-draw-and-settlement.mjs
import pg from 'pg';
...
async function main() {
  ...
  try {
    ...
    console.log(`期數 ${periodNumber} 的未結算下注記錄:`, betsResult.rows.length);
    ...
    const now = new Date();
    await client.query(
      'INSERT INTO draw_records (period, result, draw_time, created_at) VALUES ($1, $2, $3, $4)',
      [periodNumber, JSON.stringify(drawResult), now, now]
    );
    ...
    for (const bet of betsResult.rows) {
      ...
      // 檢查是否中獎
      const isWin = position < drawResult.numbers.length && drawResult.numbers[position] === betValue;
