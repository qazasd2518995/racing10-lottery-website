...
async function testAgentHierarchicalAnalysis() {
    ...
    try {
        ...
        const response1 = await axios.get(`${BASE_URL}/api/agent/reports/agent-analysis`, {
            headers: {
        ...
        if (response1.data.success) {
            ...
            // 檢查完整的數據結構
            console.log('\n🔍 完整數據結構檢查:');
            console.log('response1.data keys:', Object.keys(response1.data));
            
            if (response1.data.agentInfo) {
                console.log(`👥 代理信息: ${response1.data.agentInfo.agentCount}代理 + ${response1.data.agentInfo.memberCount}會員`);
            } else {
                console.log('❌ agentInfo字段不存在');
            }
        ...
        // 測試2: 查詢所有數據（無日期篩選）
        console.log('\n📊 測試2: 查詢所有數據（無日期篩選）');
        const response2 = await axios.get(`${BASE_URL}/api/agent/reports/agent-analysis`, {
            headers: {
                'Authorization': `Bearer ${AGENT_TOKEN}`
            }
        });
